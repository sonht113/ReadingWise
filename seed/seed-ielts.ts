...
import ieltsData from "./ielts-articles.json";
...
async function seed() {
  console.log("Seeding IELTS Reading Practice database...\n");

  const existing = await db
    .select()
    .from(collections)
    .where(eq(collections.name, "IELTS Reading Practice"));
  ...
  if (existing.length > 0) {
    ...
    const [c] = await db
      .insert(collections)
      .values({ name: "IELTS Reading Practice" })
      .returning();
    console.log(`Created collection: ${c.name} (${c.id})`);
    collectionId = c.id;
  }

  for (const item of ieltsData as IeltsArticle[]) {
